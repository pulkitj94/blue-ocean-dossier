...
router.post('/users/create', requireAdmin, asyncHandler(async (req, res) => {
  const db = getDB();
  const { username, password, first_name, last_name, designation, organization } = req.body;
  ...
  try {
    const result = await db.prepare("INSERT INTO users (username,password_hash,first_name,last_name,designation,organization,role) VALUES (?,?,?,?,?,?,'respondent')").run(finalUsername, hash, first_name||'', last_name||'', designation||'', organization||'');
...
router.put('/users/:id/edit', requireAdmin, asyncHandler(async (req, res) => {
  const db = getDB();
  const { first_name, last_name, designation, organization } = req.body;
  await db.prepare('UPDATE users SET first_name=?, last_name=?, designation=?, organization=? WHERE id=? AND role=?').run(first_name||'', last_name||'', designation||'', organization||'', parseInt(req.params.id), 'respondent');
